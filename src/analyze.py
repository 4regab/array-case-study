...
class StudentAnalytics:
    def __init__(self, students: List[Dict[str, Any]]):
        ...
        self.final_grades = [s['final_grade'] for s in students if s['final_grade'] is not None]
    ...
    def get_quiz_statistics(self) -> Dict[str, Dict[str, float]]:
        ...
        for i in range(1, 6):
            quiz_key = f'quiz{i}'
            quiz_scores = [s[quiz_key] for s in self.students if s.get(quiz_key) is not None]
            if quiz_scores:
    ...
    def get_component_statistics(self) -> Dict[str, Dict[str, float]]:
        ...
        for comp in components:
            scores = [s[comp] for s in self.students if s.get(comp) is not None]
            if scores:
    ...
    def calculate_percentiles(self) -> List[Dict[str, Any]]:
        ...
        for student in self.students:
            ...
            student_copy['percentile_rank'] = below_count + 1  # Rank starting from 1
    ...
    def get_all_outliers(self) -> Dict[str, Any]:
        ...
        zscore_outliers, zscore_info = self.identify_outliers_zscore(self.final_grades)
    ...
    def _get_section_letter_grades(self, section: str) -> Dict[str, int]:
        ...
        section_students = [s for s in self.students if s.get('section') == section and s.get('letter_grade')]
        letter_counts = Counter(student['letter_grade'] for student in section_students)
...
class GradeVisualizer:
    ...
    def plot_statistics_summary(self, analytics: StudentAnalytics, save_path: str = None):
        ...
        bars1 = ax1.bar(tendency_metrics, tendency_values, color=self.colors[:3], alpha=0.7)
        ...
        bars2 = ax2.bar(variability_metrics, variability_values, color=self.colors[1:4], alpha=0.7)
        ...
        # Distribution shape
        final_grades = [s['final_grade'] for s in analytics.students if s['final_grade'] is not None]
        ax3.hist(final_grades, bins=15, alpha=0.7, color=self.colors[0], edgecolor='black', density=True)
        ax3.axvline(stats['mean'], color='red', linestyle='--', label=f'Mean: {stats["mean"]:.1f}')
        ax3.axvline(stats['median'], color='green', linestyle='--', label=f'Median: {stats["median"]:.1f}')
        ...
        ax4.set_title(f'Box Plot (Outliers: {outlier_data["iqr"]["statistics"]["outlier_count"]})')
        ax4.grid(True, alpha=0.3)
        
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'statistics_summary.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_percentile_analysis(self, analytics: StudentAnalytics, save_path: str = None):
        ...
        scatter = ax1.scatter(percentiles, final_grades, alpha=0.6, color=self.colors[0])
        ...
        ax2.hist(percentiles, bins=10, alpha=0.7, color=self.colors[1], edgecolor='black')
        ...
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'percentile_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_outlier_analysis(self, analytics: StudentAnalytics, save_path: str = None):
        ...
        ax1.axhline(iqr_stats['upper_bound'], color='red', linestyle='--', label='Upper Bound')
        ax1.axhline(iqr_stats['lower_bound'], color='red', linestyle='--', label='Lower Bound')
        ax1.set_title(f'IQR Method: {outlier_data["iqr"]["statistics"]["outlier_count"]} outliers')
        ax1.set_ylabel('Final Grade')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        z_scores = [(x - zscore_stats['mean']) / zscore_stats['std_dev'] for x in final_grades]
        ax2.scatter(range(len(z_scores)), z_scores, alpha=0.6, color=self.colors[1])
        ax2.axhline(zscore_stats['threshold'], color='red', linestyle='--', label=f'Threshold: ±{zscore_stats["threshold"]}')
        ax2.axhline(-zscore_stats['threshold'], color='red', linestyle='--')
        ax2.set_xlabel('Student Index')
        ax2.set_ylabel('Z-Score')
        ax2.set_title(f'Z-Score Method: {outlier_data["zscore"]["statistics"]["outlier_count"]} outliers')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'outlier_analysis.png'), dpi=300, bbox_inches='tight')
        plt.show()
        
        # Print outlier details
        print("\n=== OUTLIER ANALYSIS ===")
        print(f"IQR Method found {len(outlier_data['iqr']['outlier_students'])} outliers")
        print(f"Z-Score Method found {len(outlier_data['zscore']['outlier_students'])} outliers")
        
        if outlier_data['iqr']['outlier_students']:
            print("\nIQR Outliers:")
            for student in outlier_data['iqr']['outlier_students']:
                print(f"  {student['name']} (Section {student['section']}): {student['final_grade']:.1f}")
    ...
    def plot_quiz_statistics(self, analytics: StudentAnalytics, save_path: str = None):
        ...
        ax2.bar(x_pos, difficulty, alpha=0.7, color=self.colors[1], edgecolor='black')
        ax2.set_xlabel('Quiz')
        ...
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'quiz_statistics.png'), dpi=300, bbox_inches='tight')
        plt.show()
    
    def plot_grade_distribution(self, students: List[Dict[str, Any]], save_path: str = None):
        """Plot histogram of final grade distribution"""
        final_grades = [s['final_grade'] for s in students if s['final_grade'] is not None]
        
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        
        # Histogram
        ax1.hist(final_grades, bins=20, alpha=0.7, color=self.colors[0], edgecolor='black')
        ...
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'grade_distribution.png'), dpi=300, bbox_inches='tight')
        plt.show()
    
    def plot_letter_grades(self, students: List[Dict[str, Any]], save_path: str = None):
        """Plot bar chart of letter grade counts"""
        letter_grades = [s['letter_grade'] for s in students if s['letter_grade']]
        grade_counts = {}
        ...
        plt.tight_layout()
        if save_path:
            plt.savefig(os.path.join(save_path, 'letter_grades.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _print_analytics_report(self, report: Dict[str, Any]):
        ...
        print(f"Students with Missing Data: {report['students_with_missing_data']}")
        ...
        print(f"\nOUTLIER ANALYSIS:")
        print(f"  IQR Method: {report['outliers']['iqr']['statistics']['outlier_count']} outliers")
        print(f"  Z-Score Method: {report['outliers']['zscore']['statistics']['outlier_count']} outliers")


# Example usage
if __name__ == "__main__":
    # These imports would need to be available in your project
    from ingest import ingest
    from transform import transform_students
    
    # Load and transform data
    raw_students = ingest()
    transformed_students = transform_students(raw_students)
    
    # Create analytical visualizations
    visualizer = GradeVisualizer()
    visualizer.generate_all_analytical_visualizations(transformed_students, "analytics_visualizations")
