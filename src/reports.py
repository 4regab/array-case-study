...
class ReportGenerator:
    ...
    def generate_summary_report(self) -> str:
        ...
        students_with_grades = [s for s in self.students if s.get('final_grade') is not None]
        ...
        for grade in ['A', 'B', 'C', 'D', 'F']:
            letter_counts[grade] = sum(1 for s in students_with_grades if s.get('letter_grade') == grade)
        
        # At-risk students
        at_risk_threshold = self.config.get('thresholds', {}).get('at_risk', 60)
        at_risk = [s for s in students_with_grades if s['final_grade'] < at_risk_threshold]
        ...
        report.append(f"  Students with Complete Data: {len(students_with_grades)}")
        report.append(f"  Students with Missing Data: {total_students - len(students_with_grades)}")
        ...
        for grade, count in letter_counts.items():
            percentage = (count / len(students_with_grades) * 100) if students_with_grades else 0
    ...
    def generate_student_report(self, student: Dict[str, Any]) -> str:
        ...
        report.append(f"Name: {student.get('first_name', '')} {student.get('last_name', '')}")
        ...
        report.append(f"  Midterm: {midterm:.2f}" if midterm is not None else "  Midterm: Missing")
        report.append(f"  Final: {final:.2f}" if final is not None else "  Final: Missing")
        report.append("")
        
        report.append("ATTENDANCE:")
        attendance = student.get('attendance')
        report.append(f"  Attendance: {attendance:.2f}" if attendance is not None else "  Attendance: Missing")
        report.append("")
    ...
    def generate_section_report(self, section: str) -> str:
        ...
        section_students = [s for s in self.students if s.get('section') == section]
        students_with_grades = [s for s in section_students if s.get('final_grade') is not None]
        students_missing_grades = [s for s in section_students if s.get('final_grade') is None]
        ...
        report.append(f"  Students with Complete Data: {len(students_with_grades)}")
        report.append(f"  Students with Missing Data: {len(students_missing_grades)}")
        ...
        sorted_students = sorted(students_with_grades, key=lambda x: x['final_grade'], reverse=True)
    ...
    def generate_at_risk_report(self) -> str:
        ...
        at_risk_threshold = self.config.get('tresholds', {}).get('at_risk', 60)
        students_with_grades = [s for s in self.students if s.get('final_grade') is not None]
        at_risk = [s for s in students_with_grades if s['final_grade'] < at_risk_threshold]
        ...
        if at_risk:
            ...
            for i, student in enumerate(at_risk, 1):
                ...
                report.append(f"   Final Grade: {student['final_grade']:.2f} ({student['letter_grade']})")
                
                # Show weak areas
                report.append("   Component Breakdown:")
                if student.get('quiz_avg') is not None:
                    report.append(f"     Quiz Average: {student['quiz_avg']:.2f}")
                ...
                if student.get('attendance') is not None:
                    report.append(f"     Attendance: {student['attendance']:.2f}")
        else:
    ...
    def generate_top_performers_report(self, top_n: int = 10) -> str:
        ...
        students_with_grades = [s for s in self.students if s.get('final_grade') is not None]
        top_students = sorted(students_with_grades, key=lambda x: x['final_grade'], reverse=True)[:top_n]
    ...
    def save_all_reports(self, output_folder: str):
        """Save all reports to text files"""
        ...
        # At-risk report
        with open(os.path.join(output_folder, 'at_risk_report.txt'), 'w', encoding='utf-8') as f:
            f.write(self.generate_at_risk_report())
        
        # Top performers report
        with open(os.path.join(output_folder, 'top_performers_report.txt'), 'w', encoding='utf-8') as f:
            f.write(self.generate_top_performers_report())
        
        # Section reports
        sections = set(s.get('section') for s in self.students if s.get('section'))
        for section in sections:
            with open(os.path.join(output_folder, f'section_{section}_report.txt'), 'w', encoding='utf-8') as f:
                f.write(self.generate_section_report(section))
        
        # Individual student reports
        student_reports_folder = os.path.join(output_folder, 'individual_reports')
        if not os.path.exists(student_reports_folder):
            os.makedirs(student_reports_folder)
        
        for student in self.students:
            # Create a unique identifier if student_id is missing
            student_id = student.get('student_id')
            if student_id is None:
                student_id = f"{student.get('last_name', 'unknown')}_{student.get('first_name', 'unknown')}"
            filename = f"student_{student_id}.txt"
            with open(os.path.join(student_reports_folder, filename), 'w', encoding='utf-8') as f:
                f.write(self.generate_student_report(student))
        
        # Export JSON
        self.export_to_json(os.path.join(output_folder, 'students_data.json'))
...
if __name__ == '__main__':
    # This is a test - normally you'd import from other modules
