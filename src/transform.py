# transform.py
from ingest import ingest  # Import the ingest function from the ingestion script

# Computes the weighted final grade
# Weights: Quizzes 30%, Midterm 30%, Final 30%, Attendance 10%
def compute_final(quiz_avg, midterm, final, attendance):
    ...
    weighted_quiz = quiz_avg * 0.30
    weighted_midterm = midterm * 0.30
    weighted_final = final * 0.30
    weighted_attendance = attendance * 0.10
...
# Modifies records in-place and returns the array
...
if __name__ == '__main__':
    # Ingest the data from the CSV (using the provided ingest function)
    students = ingest()
    
    # Transform the data by adding computed fields
    transformed_students = transform_students(students)
    
    # Print the transformed records (now including quiz_avg, final_grade, letter_grade)
